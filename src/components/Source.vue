<template>
  <div class="share mt-1">
    <b-button-group>
      ...
      >
        ...
        >
          ...
          <b-icon-exclamation-triangle
            ...
            :title="
              l.ui
                ? $t('source.language.onlyUI')
                : $t('source.language.onlyData')
            "
            class="ml-2"
          />
        </b-dropdown-item>
      ...
      >
        <b-icon-link />
        <span class="button-label">{{ $t("source.label") }}</span>
      </b-button> -->
    ...
    >
      <Url
        id="browserUrl"
        :url="browserUrl()"
        :label="$t('source.share.sharePageWithOthers')"
        :open="false"
      />
      <hr />
      <b-button class="twitter mr-1" :href="twitterUrl"
        ><b-icon-twitter /> {{ $t("source.share.twitter") }}</b-button
      >
      <b-button variant="dark" :href="mailTo"
        ><b-icon-envelope /> {{ $t("source.share.email") }}</b-button
      >
    </b-popover>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    showRoot() {
      ...
      return (
        (Array.isArray(this.conformsTo) && this.conformsTo.length > 0) ||
    ...
    languages() {
      ...
      for (let lang of this.dataLanguages) {
        if (
          !Utils.isObject(lang) ||
          !lang.code ||
          this.supportedLocales.includes(lang.code)
        ) {
          continue;
        }
        let newLang = {
      ...
      if (this.supportsExtension(LANGUAGE_EXT)) {
        ...
        const dataSupported = prepareSupported(
          this.dataLanguages.map((l) => l.code)
        );
